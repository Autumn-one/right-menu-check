...
每次试验启动一个新的、非提权、STA worker，并在发送请求前加入 kill-on-close Job Object。默认执行 3 次，可选 3、5、7、10、15 或 20 次；每次都有独立 PID。超时、进程崩溃、COM激活失败和接口失败分别统计。
...
- 详情面板按失败结果、失败阶段、错误类型、HRESULT和消息分组；同一原因会显示出现次数。

每次使用新进程可避免模块已在同一进程加载，但不会清空 Windows文件缓存，因此界面称为“新进程试验”，不称为绝对冷启动。

## 整体菜单基准

整体基准在另一个隔离 worker中调用 `SHCreateDefaultContextMenu` 和 `QueryContextMenu`，用于测量给定文件、文件夹或空白处的完整 Shell菜单构建。它可以揭示单项探针之和不能解释的总体成本，但不会调用菜单命令，仍不是 Explorer窗口从鼠标事件到画面出现的绝对 UI延迟，也不能单独指出某一个扩展是原因。
